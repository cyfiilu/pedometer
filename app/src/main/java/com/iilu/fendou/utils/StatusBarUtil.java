...
public class StatusBarUtil {
    ...
    public static void compat(Activity activity, int statusColor) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (isEMUI() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
                // 华为5.0.2/5.1.1
                window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
                window.getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_VISIBLE);
            } else {
                // vivo5.0.2，小米5.0.2/6.0.1，华为6.0/7.0，魅族5.1，三星7.0
    ...
    public static void setStatusTextColor(boolean useDart, Activity activity) {
        if (isFlyme()) {
            ...
            activity.getWindow().getDecorView().findViewById(android.R.id.content).setPadding(0, 0, 0, /*getNavigationBarHeight(activity)*/0);
        }
    }
     ...
     *    为了解决这个问题，小米觉决定在MIUI 9之后兼容Android 官方的标准方案，舍弃了自己的方案
     * 3、在新的 MIUI 版本（即基于 Android 6.0 ，开发版 7.7.13 及以后版本）
     */
