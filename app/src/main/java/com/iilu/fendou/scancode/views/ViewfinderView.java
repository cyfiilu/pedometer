package com.iilu.fendou.scancode.views;
...
public final class ViewfinderView extends View {
    ...
    public ViewfinderView(Context context, AttributeSet attrs) {
        ...
        // Initialize these once for performance rather than calling them everytime in onDraw().
        paint = new Paint();
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (resultBitmap != null) {
            ...
            if (textType != TEXTCONTENTTYPE_REGISTER) {
                ...
                canvas.drawRect(frame.left, frame.top, frame.left + 30, frame.top + 7, paint);
                canvas.drawRect(frame.left, frame.top, frame.left + 7, frame.top + 31, paint);
                canvas.drawRect(frame.left, frame.bottom - 7, frame.left + 30, frame.bottom + 1, paint);
                canvas.drawRect(frame.left, frame.bottom - 30, frame.left + 7, frame.bottom + 1, paint);
                canvas.drawRect(frame.right - 7, frame.top, frame.right + 1, frame.top + 30, paint);
                canvas.drawRect(frame.right - 30, frame.top, frame.right + 1, frame.top + 7, paint);
                canvas.drawRect(frame.right - 7, frame.bottom - 30, frame.right + 1, frame.bottom + 1, paint);
                canvas.drawRect(frame.right - 30, frame.bottom - 7, frame.right + 1, frame.bottom + 1, paint);
            }
            ...
            canvas.drawBitmap(((BitmapDrawable) (getResources().getDrawable(R.mipmap.code_scan_line))).getBitmap(),
                    null, lineRect, paint);
            // 画扫描框下面的字 换行方法
            ...
            StaticLayout layout = new StaticLayout(scanContent, textPaint,
                    frame.right - frame.left, Alignment.ALIGN_CENTER, 1.5F, 0.0F, true);
            // 从(20,80)的位置开始绘制
